...
class AlignmentAviary(BaseSingleAgentAviary):
    ...
    def step(self, action):
        ...
        if action == 0:
            action_ = np.array([[0, 0, 0, 0]])
        elif action == 1:
            action_ = np.array([[1e-3,1e-3,-1e-3, -1e-3]])
        elif action == 2:
            action_ = np.array([[-1e-3,-1e-3, 1e-3, 1e-3]])
        elif action == 3:
            action_ = np.array([[-1e-3, 1e-3, 1e-3, -1e-3]])
        ...
        if action == 1:
            # obs, r, d, info = super().step(0)
            # reward += r
            # done |= d
            action_ = np.array([[-1e-3,-1e-3, 1e-3, 1e-3]])
            obs, r, d, info = super().step(action_)
            reward += r
            done |= d
